...
describe("validateStatus", () => {
  it("does not throw when status matches", () => {
    expect(() => validateStatus(200, 200, "http://example.com", "ok")).not.toThrow();
  });
  it("does not throw for 200/304 equivalence", () => {
    expect(() => validateStatus(200, 304, "http://example.com", "ok")).not.toThrow();
  });
  it("throws when status does not match", () => {
    expect(() => validateStatus(200, 403, "http://example.com/app", "Forbidden")).toThrow(
      "Status 403 received, expected 200",
    );
  });
  it("includes URL in error message", () => {
    expect(() => validateStatus(200, 500, "http://example.com/app", "err")).toThrow(
...
describe("extractWorkerId", () => {
  it("extracts worker ID from base href", () => {
    const html = '<html><head><base href="_w_abc123/"></head><body>...</body></html>';
