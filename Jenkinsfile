...
try {
  timestamps {
    node('docker') {
      ...
      stage('s3 upload') {
        sh """
        aws s3 cp target/shinycannon-1.0-jar-with-dependencies.jar s3://rstudio-shiny-server-pro-build/shinycannon/shinycannon-\$(date +"%Y-%m-%d")-\$(git rev-parse --short=7 --verify HEAD).jar
        """
      }
    }
