...
const { parse, resolve: urlResolve } = require("url");
...
class EventContext extends EventEmitter {
  ...
  async nextMessage() {
    ...
    return new Promise((resolve, reject) => {
      const msgHandler = msg => {
        if (!shouldIgnore(msg)) {
          this._websocket.removeListener("message", msgHandler);
  ...
  resolve(url) {
    return urlResolve(this.target, url.replace(/^\//, ""));
  }
}
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = protocol.request(urlObj, async res => {
        if (
          ...
          !(
            (this.statusCode == 304 && res.statusCode == 200) ||
            (this.statusCode == 200 && res.statusCode == 304)
          )
        ) {
...
class ShinyHomeRequestEvent extends ShinyRequestEvent {
  ...
  async handleResponse(ctx, res) {
    ...
    return new Promise((resolve, reject) => {
      res.on("data", chunk => {
        writer.write(chunk);
        const re_ssp = /<base href="_w_([0-9a-z]+)\/" \/>/i;
        const re_rsc = /<base href="_w_([0-9a-z]+)\/"\/>/i;
        for (let re of [re_ssp, re_rsc]) {
          let m = re.exec(chunk.toString());
          if (m !== null) {
      ...
      res.on("end", () => {
        if (!ctx.tokens.hasToken("WORKER")) reject("WORKER id not found (or too many!)");
        resolve(writer.toString());
      });
      res.on("error", reject);
    });
...
class ShinyWSOpenEvent extends ShinyEvent {
  ...
  async execute(ctx) {
    ...
    return new Promise((resolve, reject) => {
      ...
      wsclient.connect(url.replace(/^https:/i, "wss:"));
    });
  }
}
    ...
    if (!messageMatch(this.message, msgTemplate)) {
      throw new Error(
        "Messages do not match!\n\n" +
          `Expected:\n${this.message.substr(0, 20) + "..."}\n\n` +
          `Actual:\n${msgTemplate.substr(0, 20) + "..."}`
      );
    }
  }
