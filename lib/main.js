...
const argv = require("yargs")
  ...
  .command("playback <file>", "Playback a run", yargs => {
    ...
    yargs.options("duration", {
      alias: "d",
      describe: "Duration of test in seconds",
      default: 1
    });
...
async function startPlayback(file, concurrency, nruns, target) {
  ...
  const duration = argv.duration + argv.concurrency * argv.startDelayMs;
  const plays = [];
  for (let i = 0; i < argv.concurrency; i++) {
    const startDelayMs = i * argv.startDelayMs;
    plays.push(
      run(jsonEvents, startDelayMs, i, duration, reporter, argv.target)
    );
  }
